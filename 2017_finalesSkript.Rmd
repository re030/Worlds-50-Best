...
edges <- read.csv("https://raw.githubusercontent.com/re030/Worlds-50-Best/master/2017_Edgelist.1.4.csv", header=T, as.is=T)

nodes <- read.csv("https://raw.githubusercontent.com/re030/Worlds-50-Best/master/2017_Nodelist.1.3.csv", header=T, as.is=T)
...
#Koch=1=blau, Ausbilder=2=indianred, KochundAusbilder=3=seagreen, Liste=4=white
colrs <- c("blue", "indianred", "seagreen", "white")
type <- c("1", "2", "3", "4")
...
plot(chef, vertex.label=V(chef)$name,
     ...
     vertex.size=70,
     edge.arrow.size=.1,
     main="2017 Gesamtnetzwerk der besten Köche weltweit")
```
...
outds <-degree(chef, mode="out")
outds

```

```{r}
egoducasse <- make_ego_graph(chef, order=1, c("Alain Ducasse"))
plot(egoducasse[[1]], 
     edge.arrow.size=.3,
     col=colrs,
     vertex.label.family="Helvetica",
     ...
     vertex.size=5,
     edge.arrow.size=.3,
     main="2017 Egonetzwerk Alain Ducasse")
```
...
plot(tnchefoamddrei_relations, layout=layout_with_kk,
     ...
     main="Darstellung aller Köche die auf den Listen sind",
     layout=coords,
     rescale=FALSE,
     vertex.label.color="black",
     ylim=c(-5,4),
...
plot(tnchefoamddrei_relations, 
     ...
     vertex.size=5,
     edge.arrow.size=.1,
     main="2017 Wer steht auf allen Listen?")
```
...
plot(mplus3,
     ...
     vertex.size=5,
     edge.arrow.size=.1,
     main="2017 Köche mit 3 Michelin Sternen")

# jetzt haben wir alle 3 Sterne Köche
```

```{r Köche ausgebildet von Ducasse mit mindestens 2 Sternen}
#Wie erfolgreich sind die Köche, die von dem Koch mit den meisten Schülern (bekanntester Koch) ausgebildet wurden?

egoducasse <- make_ego_graph(chefol, order=1, c("Alain Ducasse"))
...
plot(egoducasse[[1]], 
     ...
     vertex.size=5,
     edge.arrow.size=.1,
     main="2017 Alain Ducasse und seine Schüler mit Michelinsternen",
     )
```
...
chef
bestchef <- delete_vertices(chef, V(chef)[worldsfifty == "99" & lechef =="99" & tripadvisor =="99" & thebestchef =="99"])
...
plot(egowfb[[1]], 
     ...
     vertex.size=5,
     edge.arrow.size=.1,
     main="2017 Egonetzwerk Worlds 50 Best")
...
plot(egotad[[1]], 
     ...
     vertex.size=5,
     edge.arrow.size=.1,
     main="2017 Egonetzwerk Tripadvisor")
...
plot(egotbc[[1]], 
     ...
     vertex.size=5,
     edge.arrow.size=.1,
     main="2017 Egonetzwerk The Best Chef")
```
...
plot(m0,
     main="2017 Köche ohne Michelinsterne",
     col=colrs,
     vertex.label.family="Helvetica",
     vertex.label.dist=1,
     vertex.label.color="black",
